...
def _domain_name_from_video_path(video_path):
    parts = _path_parts(video_path)
    if len(parts) < 4:
...
class FaceDataset(Dataset):
    def __init__(self, root_dir, data_names, phase='test', transform=None, verbose=False):
        ...
        if verbose:
            print('--------------------------------------------------')
            print(f'{"build test datasets":20} - number of video')
            print('--------------------------------------------------')
    ...
    def __getitem__(self, idx):
        ...
        domain_label = self.data_names[_domain_name_from_video_path(video_name)]
...
class BalanceFaceDataset(Dataset):
    def __init__(self, root_dir, data_names, phase='train', transform=None, max_iter=4000, verbose=False):
        ...
        for num, data_name in enumerate(data_names):
            ...
            path = os.path.join(root_dir, data_name, phase, 'live')
            video_list = [os.path.join(path, video) for video in os.listdir(path)]
            random.shuffle(video_list)
            self.video_list[path] = video_list
        ...
        if verbose:
            print('--------------------------------------------------')
            print(f'{"build train datasets":20} - number of video')
            print('--------------------------------------------------')
    ...
    def __getitem__(self, idx):
        ...
        for video_key in self.video_list:
            ...
            video_key_parts = _path_parts(video_key)
            key = f"{video_key_parts[-3]}-{video_key_parts[-1]}"
            sample[key] = {
